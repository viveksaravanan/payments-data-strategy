...
All v2.5 agents are merchant-scoped — every query inherits a viewing-merchant context. Strategy doc §10.2 specifies seven merchant-scoped specialist personas; the demo ships the Conversational Business Advisor (Merchant Advisor) and treats the others as roadmap. There is no network-level analyst agent in v2.5.
    ...
    └────┬────┘ └────┬────┘ └─────┬───────┘ └────┬─────┘ └────┬─────┘
         ...
         └──────────┴─────────────┴──────────────┴────────────┘
              ...
              └─────────────┬─────────────────────┘
                            ▼
              ┌───────────────────────────────────┐
              │  Merchant Advisor agent           │   insight (§10)
              │  Streamlit dashboard              │
...
| §10.2 Conversational Business Advisor | **Built as Merchant Advisor (one of the seven personas).** | The other six are roadmap; the build cost is incremental rather than architectural. |
| §10.2 Demand Forecasting / Pricing / Segmentation / Trade Area / Payment Opt / Anomaly | (not built) | Roadmap. The advisor's lake queries cover some segmentation and benchmarking on demand. |
...
1. **Add the remaining six agents** from strategy doc §10.2. The pattern is shared (system prompt + tools + loop); each agent is incremental work.
...
- *"One of the seven agents from §10.2 is built — the Conversational Business Advisor specialized as a Merchant Advisor. The other six follow the same architectural pattern; the build cost is incremental rather than architectural."*
