...
# Peer display labels
#
# The datamodel-v2 line-item lake publishes a single aggregate peer
# (``peer_relationship = 'peer'``), so there is one internal peer key,
# ``peer_aggregate``. User-facing strings — legends, axis ticks, hover
# tooltips, takeaway subtitles — render it as "Peers (avg)". The demo
# framing is "you vs the market", never "you vs ACM specifically".
# (The old per-competitor `peer_a` / `peer_b` pseudonyms are gone.)
# ---------------------------------------------------------------------------
...
def peer_display(label: str) -> str:
    """Convert an internal peer id (``peer_aggregate``) to its
    user-facing display form (``Peers (avg)``). Unknown labels fall
    through unchanged."""
...
# ---------------------------------------------------------------------------
#
# Used by QSR questions whose comparison axis is not a peer cohort
# but a within-merchant breakdown (per-daypart, per-category). Takes
# an arbitrary list of own series with distinct line styles.
