...
5. **All analytical fields raw:** `unit_price`, `qty`, `discount`, `line_total`, `department`, `category`, `subcategory`, and payment dims all kept unbinned. Protection comes from the query-time floor, not from coarsening values. The taxonomy (`department`/`category`/`subcategory`) is the **shared functional hierarchy** resolved from the `products` join — the cross-merchant comparison key. A merchant's own (divergent) `merchant_*` labels are never read into the lake (datamodel-v2 dual taxonomy). Agents use functional taxonomy for peer comparison and their own `merchant_*` labels only for own-data detail.
