...
- The count-floor is a **second** line of defense, not the primary one: grouping by `lake_txn_id` produces count-1 groups that fail `k=5` and drop. But "most attempts suppress themselves" is not "all" — a non-grouped raw select can still surface a `unit_price` shared by ≥5 lines. The AST aggregating-only check above is what makes raw-row exposure *structurally* impossible; the floor handles the thin-cell case.
- **MIN/MAX and percentile-type functions are ALLOWED for now** (decision #2) — accepted small exposure of a single real value. Flagged for later tightening: *"ban single-row-exposing functions on raw value columns when privacy posture is raised."*
