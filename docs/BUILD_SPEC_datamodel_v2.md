...
> **Theme-5 placeholder (planned, NOT built in v2).** Theme 5 will add a `raw_description` column to `transaction_items` — the messy/truncated terminal descriptor string captured per line — and derive the clean catalog (product_name, categories, functional taxonomy) from `sku` + `raw_description`, removing the assumed-normalized clean catalog for own AND peer. **In v2 the line stays minimal: `sku` + qty + price only** (no `raw_description`); product_name/taxonomy resolve via the `sku`→`products` join. This note documents the seam so the schema change is anticipated, not a surprise.
